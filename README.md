...
- Lion Mac (Mac Pro 3,1 Early 2008, operator-confirmed 2026-09-12 / 10.6.8): `docs/lion-ssd-handoff.md` + `docs/lion-workflow.json`. Current phone links: `da.gd/lionrelay` (relay page) and `da.gd/lionzip` (current bundle). Legacy `da.gd/lzr`, `da.gd/lmz`, and `da.gd/lpg` are not the current session path. Never `da.gd/lionfix`, `da.gd/lup`, or TinyURL. Do not erase Lion SSD Base or start disk clone.
