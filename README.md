...
Second physical machine (keep-as-Mac): Mac Pro 3,1 Early 2008 (A1186 family; operator-confirmed 2026-09-12 with photo — MASTER's old "1,1" refs were stale), Lion 10.7.5 now booted from `start disk clone` (LIONONE1 receipt 2026-09-16: `/dev/disk3s2` mounted at `/`; physical bay unverified), Arctic Fox 47.3. Old wipe directions are suspended; protect boot and all RAID members. Lion-from-SSD goal, links, and nextAction: `docs/lion-ssd-handoff.md` / `node tools/lion-status.ts`.
...
- Lion Mac (Mac Pro 3,1 Early 2008, operator-confirmed 2026-09-12 / 10.6.8): `docs/lion-ssd-handoff.md` + `docs/lion-workflow.json`. Current phone link: `da.gd/sQ7bEo` (One page + bundle + receipt Burn). `da.gd/lionrelay` and `da.gd/lionzip` are legacy. Legacy `da.gd/lzr`, `da.gd/lmz`, and `da.gd/lpg` are not the current session path. Never `da.gd/lionfix`, `da.gd/lup`, or TinyURL. Do not erase Lion SSD Base or start disk clone.
