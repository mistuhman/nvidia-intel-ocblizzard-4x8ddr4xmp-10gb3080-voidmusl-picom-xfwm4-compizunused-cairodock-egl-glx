...
- T1 activation + merge receipts (2026-09-16): operator pasted .github/workflows/mac-es5-passthrough.yml straight to main (commit 0450afe) - workflow REGISTERED active id 359758529, Run button live on phone mobile web. Pasted copy = PRE-hardening YAML (choice input, no all-default, no pinned checkout, no corpus snapshot step): runs still work post-merge (main now carries the tool incl. corpus v1) but sync to current ci source pending - routes: grant Arena app Workflows permission (agent maintains .github forever, recommended) or re-paste via web UI edit from the raw ci URL. PR #88 MERGED = d57fb8f per operator directive (main carries tools + docs + ci sources + activation). T1 closes on: first CI run receipt + on-Mac burn.
