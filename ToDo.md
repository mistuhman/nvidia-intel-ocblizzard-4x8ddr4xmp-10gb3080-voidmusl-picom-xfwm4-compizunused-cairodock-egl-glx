...
- [~] 2. **Clear Keychains / User Data / Account**: IN PROGRESS 2026-09-15 - operator deleting second admin `ddds` via Users & Groups (GUI shows "Deleting Account..."; slow = Lion's delete-with-disk-image/secure-erase pass over the home folder; let it finish, do not force-quit). Keychain sweep rides along with `lion-harden`.
- [ ] 3. **Drive Swap + RAID**: Wipe/format the 3x Western Digital 1TB drives (Bays 2, 3, 4) in Disk Utility for clean storage/RAID. REGISTERED 2026-09-15 as gated blocks, one step at a time: ?(lion-erase-inventory-probe) READ-ONLY first -> operator pastes full output -> agent arms ?(lion-erase-spares-wd1tb) with probe-confirmed node ids (KEEP-list hard guard: boot disk, the HDD carrying the iTunes backup, and the Bay 3 clone NEVER appear in the erase list; music extraction from the WDs is operator's call BEFORE arming) -> Disk Utility RAID after the erase -> ?(lion-harden) config-only on the boot volume (firewall+stealth, guest off, remote login off, screensaver password, update chatter off; never touches /Applications so Arctic Fox is kept by rule).
