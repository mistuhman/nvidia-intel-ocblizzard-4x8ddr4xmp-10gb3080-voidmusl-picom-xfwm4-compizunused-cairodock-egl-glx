...
const INSTRUCTIONS: { version: string; common: string[]; apps: Record<string, string[]> } = {
  ...
  apps: {
    ...
    "vencord-web census after the window and class rules (2026-09-16k): same pinned bytes, 181 hazards = 51 mid-pattern lookbehind + 20 unbounded-width lookbehind + 18 capture-in-lookbehind + 90 clone-escape + 2 unicode-property, 0 false unparseable. 21 patterns left the 'width' class: they now rewrite, or are refused for the real reason (a lookbehind that captures, or an object that reaches matchAll). The app stays BLOCKED by USE, not by capability - the same lookbehinds compile when the regex reaches only replace/split/match/search, which is why COMPATIBILITY.json quotes every blocked pattern with its sink",
