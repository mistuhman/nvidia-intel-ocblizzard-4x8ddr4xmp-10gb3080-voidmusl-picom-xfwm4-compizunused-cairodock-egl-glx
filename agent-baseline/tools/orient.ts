#!/usr/bin/env node
// Ground truth for a new chat session: branch, head, status, file map, and the live
// objective from MASTER.md. Deterministic: fixed section order, sorted file lists,
...
function printObjective(master: { raw: string; json?: Master }): void {
  if (!master.json?.objective) {
    console.log('ACTIVE OBJECTIVE not found (MASTER.md missing or not JSON)');
    return;
  }
...
function main(): void {
  ...
  console.log('\nfiles:');
  for (const file of ['README.md', 'MASTER.md', 'STATE.md', 'ToDo.md']) console.log(`- ${fileSummary(file)}`);
  console.log('\nscripts:');
