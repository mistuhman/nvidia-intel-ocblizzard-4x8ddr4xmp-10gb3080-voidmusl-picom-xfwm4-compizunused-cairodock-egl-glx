#!/usr/bin/env node
// Bounded agent fan-out plan for one objective. One agent per source set, hypothesis,
// or verification target; the printed JSON is the merge contract (receipts only).
...
agents.push({ id: 'context', task: `Reduce ${objective} to current facts and missing gates`, method: 'read README.md, MASTER.md, STATE.md, ToDo.md, git state', inputs: ['README.md', 'MASTER.md', 'STATE.md', 'ToDo.md', 'git status'], output: 'fact list with receipts only', gate: 'no claim without path/hash/output' });
agents.push({ id: 'gate', task: 'Current objective gate plus available block inventory', method: 'node tools/next-gate.ts', inputs: ['MASTER.md', 'etc/*.block'], output: 'ordered live gates plus available blocks', gate: 'one wave at a time; a second wave only after the first wave receipt is read' });
