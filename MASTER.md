{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "RECOVERY COMPLETE - the OMEN boots clean to Void, no Enter press, pump 1573, every fan sensor live. Next, in order and one at a time: (1) shut down, cord out, hold power 20s, REINSTALL THE PSU into the chassis, re-verify 24-pin SPWR + both CPU 4-pins + PB + both GPU 6+2 before the cord goes back in; (2) LED hub M82868-001 reconnected ALONE, one power-on - instant cycle returning finally attributes the original fault to that branch, booting clean clears it; (3) settle AHCI vs RAID before SATA storage; (4) storage one drive per power-on; (5) verify memory is XMP 3733 or stock with no 4000 remnant; (6) resume extreme-optimization-oc. Do not re-enable the 4000 memory profile. Do not add multiple variables per power-on. The original fault remains UNATTRIBUTED unless step 2 reproduces it.",
