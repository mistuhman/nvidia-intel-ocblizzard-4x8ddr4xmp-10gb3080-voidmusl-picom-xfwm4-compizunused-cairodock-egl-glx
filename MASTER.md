{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "FINAL migration gate OPEN 2026-08-21: operator reboots (runit-init 6), machine must boot hands-off to nvme/ROOT/void via the NVMe ZBM (fallback entry dead, single-boot by design), then pastes migration-final-check probe from a fresh root shell. PASS = findmnt / nvme/ROOT/void + df -h /mnt/games shows tank/games mounted + ls /mnt/games full library + pools clean. THEN mark phase7 migration COMPLETE in bootGate/currentState: operator objective achieved - root+zbm+zfs on NVMe 953.9G (512M ESP + one pool), 2TB as tank central storage (sda1 NTFS 50 backup untouched + tank/games 460G). Remaining operator-optional: delete nvme-games.tar from sda1 to reclaim 555G, reconcile sda1 Games folder overlap, then phase7 continues (doas hardening, sudo removal, logging, network control).",
