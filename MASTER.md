{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OPERATOR GATE: ladder in progress, PSU PROVISIONALLY EXONERATED. L3=C (nothing at all with both CPU 4-pins out - ambiguous, do not over-read). A1: the OMEN PSU M83827-001 powers the BENCH board and HOLDS - 'fan spins and stays spinning, no post' - so it starts on request and does not latch. NEXT TWO, in order: (1) read the bench board's 2-DIGIT POST CODE display and whether it changes on power-up - first real instrument in this campaign; (2) L2 on the OMEN, never yet run cleanly: 24-pin PLUS both 4-pin CPU plugs and NOTHING else on the PSU (no GPU, no SATA/Molex, no lighting board), one button press. Still cycling closes the accessory class and convicts board/CPU; not cycling means one accessory branch is the fault, re-add one per power-on. Test B (bench Thermaltake Smart 550W into the OMEN board) stays GATED: old budget group-regulated unit, no bench baseline yet. Jumper and USB+hotkey classes remain CLOSED. Lighting the USB requires POST: promise diagnosis, not a fix.",
