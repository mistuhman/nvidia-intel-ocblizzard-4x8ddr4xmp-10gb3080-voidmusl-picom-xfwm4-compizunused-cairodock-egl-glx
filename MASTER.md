{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "RECOVERY GATE - THE MACHINE POSTS. Order: (1) cord out, reconnect the AIO pump + CPU fan headers and the rest of the cooling - 90B is a REAL warning, do not run or load the CPU with cooling disconnected; (2) power on, confirm 90B is gone, press F10 for BIOS; (3) in BIOS load defaults, then set memory to XMP 3733 - HP's own factory DIMM M85222-001 is a DDR4-3733 part - and NEVER the 4000 custom profile that preceded the crisis; (4) Escape = ZBM, boot nvme/ROOT/void; (5) only then re-add the remaining accessory branches ONE POWER-ON AT A TIME (lighting board M82868-001, SATA/drives, fan hub, front panel) to attribute the original trip. Do not write a cause into the ledger: the fix is UNATTRIBUTED (multi-variable). After the desktop is up: run the post-recovery probes before resuming extreme-optimization-oc, keep efivarfs ro, and do not re-enable the 4000 memory profile.",
