{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OPERATOR OPT-IN GATE (pass 3). DO NOT POWER ON until the operator names the step. Jumper and USB+hotkey classes remain CLOSED - do not relabel them. NEW named class: PSU-side load isolation, docs/open-classes-pass3.md. One step at a time, one report each: L0 = zero-power photos of the PSU label and the PSU cable fan-out; then L1 = PSU alone, disconnected from everything, paperclip 24-pin pin16 green to pin17 black with a fan as load; then L2 = board only, 24-pin + both 4-pin EPS, every accessory off the PSU; then L3 = both 4-pin EPS unplugged (decisive: still cycling with the CPU unpowered kills the firmware hypothesis). Cap stays on the left pair and the cord stays out between steps. Lighting the USB requires POST - promise diagnosis, not a fix. After a real recovery only: Escape=ZBM, F10=BIOS, 3733 XMP, efivarfs ro.",
