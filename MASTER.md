{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OC meter phase. Operator gate order: (1) paste oc-p6-install (root) + oc-p5-safety-root (root) for safety/power inspection, (2) log out/in so Coolbits goes live, (3) paste oc-p5-safety-user and report PSU label + 6+2 plug count, (4) paste oc-p7-baseline for STOCK official scores, (5) GWE step 1 via oc-p8-gwe-oc. CPU = BIOS per oc-cpu-bios-checklist.md only.",
