{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OPERATOR GATE: run A1b, the single-variable PSU swap. A0 PASS: the bench POSTs on its own Thermaltake Smart 550W (boot-device menu, Kingston SV300S37A240G, POST code 02) - it is a known-good reference with a POST-code instrument. A1 as run is VOID because the GPU was absent in that pass and present in A0; do not cite A1's 'no post'. A1b: keep the exact A0 configuration that just POSTed and change ONLY the PSU to the OMEN M83827-001; report whether the boot-device menu appears and what the 2-digit code reads. Then L2 on the OMEN: 24-pin PLUS both 4-pin CPU plugs, nothing else on the PSU, one button press. Test B (Thermaltake into the OMEN board) is now less gated because the bench PSU is proven working, but it still waits for A1b and an explicit operator opt-in. Jumper and USB+hotkey classes remain CLOSED. Lighting the USB requires POST: promise diagnosis, not a fix.",
