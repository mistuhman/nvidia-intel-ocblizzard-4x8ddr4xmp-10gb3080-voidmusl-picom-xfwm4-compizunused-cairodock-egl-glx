{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "RECOVERY ESSENTIALLY DONE - BIOS reachable, Void boots, pump 1565 RPM. Remaining: (1) plug the spare F FAN into FFAN1 (bottom edge) so Front Fan #1 stops reading N/A - that is the only dead sensor and the cause of the 90B prompt; if no fan is physically mounted there, disable System Fan Check instead; (2) in Advanced verify memory is XMP 3733 or stock and that NO remnant of the 4000 custom profile survives; (3) confirm a clean boot with no Enter press. THEN, separately: reconnect the M82868-001 LED hub alone and power on - if the instant cycle returns, the hub was the original fault and can stay unplugged permanently. Storage stays deferred until AHCI-vs-RAID is decided (SATA Emulation currently = RAID, which can hide SATA disks from Linux) and only one drive per power-on. After that, resume extreme-optimization-oc from the parked state.",
