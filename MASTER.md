{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "Wave-3 gate (dispatched 2026-08-24, six blocks, paste one at a time in this order and paste outputs back): 1) etc/w3-reposync-root.block in a separately-entered root shell; 2) etc/w3-proton-declutter.block; 3) etc/w3-lunar-logs.block; 4) etc/w3-steam-tmp-clean.block (Steam fully quit first); 5) etc/w3-plutonium-prefix.block (long: dotnet48 alone takes 10-30 min); 6) etc/w3-cod4x-promod.block. Wave-4 (Lunar fix after hs_err attribution, per-game launch scripts, DXVK only if a title needs it, BO3 t7x staging) is authored only after these receipts. Operator-side in parallel, no console: Steam UI - install WaW 10090 and BO2 202970 into the /mnt/games SteamLibrary, and after the tmp-clean block run a full BO3 311210 install there (game files are gone, only crash dumps remain; ~120GB download).",
        ...
        "bootGate": "MIGRATION COMPLETE 2026-08-21 (carried, durable): bootGate2 + persistence PASS - operator verdicts: i booted, pressed escape for zbm boot menu upon turning on. and saw the nvme dir, it worked; just rebooted and let it do its thing; findmnt / = nvme/ROOT/void; BootCurrent 0002 direct NVMe ZBM; BootOrder 0002,0008; tank/games 460G self-mounted at boot; pools nvme+tank ONLINE no errors. games-probe.block RECEIVED 2026-08-22; wave-2 receipts RECEIVED 2026-08-24; wave-3 gate = outputs of the six etc/w3-*.block files pasted back before any wave-4 action block.",
        "afterBootGate": [
