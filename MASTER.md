{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "RECOVERY COMPLETE - Void boots repeatedly. Remaining work is the POST CPU Fan (90B) warning. Order: (1) operator runs etc/omen-90b-fan-probe.block as root and pastes the FULL output - it names which header reports 0 RPM and whether temps are actually controlled; (2) photo of the headers around the CPU socket showing which of CPU FAN / LCFAN / TFAN-LCFAN2 / FFAN1 / FFAN2 are occupied; (3) move the pump/CPU-fan lead to the header the EC watches, or reconnect the missing one; (4) F10 BIOS: load defaults, memory XMP 3733 (never 4000). DO NOT load or stress the CPU until the fan receipt is read. Then re-add the remaining accessory branches one power-on at a time (lighting board M82868-001, SATA/drives, fan hub, front panel) to attribute the original trip, then resume extreme-optimization-oc: post-recovery probes, efivarfs ro, GWE offsets, and the parked benchmarks.",
