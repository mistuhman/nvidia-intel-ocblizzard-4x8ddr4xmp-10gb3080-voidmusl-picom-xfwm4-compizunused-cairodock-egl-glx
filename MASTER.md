{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OPERATOR GATE: L0 mostly PASS, ONE ITEM OUTSTANDING. L0-A PASS (PartSurfer BOM for serial 2MO22432DX: GT22-0139 / 575Q1AA; PSU M83827-001 'POWER SUPPLY UNIT 800W ATX Gold'; board M81915-601 BlizzardOC; PCA LIGHTING CONTROL M82868-001 fitted; cooler M82880-002 plain LCS 240, no Cryo/TEC). L0-C PASS ('no damage'). OUTSTANDING AND MANDATORY: one photo of the 24-pin at the motherboard end, close and straight-on, wire colours legible, latch visible. That photo is the L1 safety gate: green wire = standard PS_ON# pin 16 and L1 proceeds; all-black harness = derive the pin index from latch/keying in the photo first, never from memory. DO NOT POWER ON, do not unplug, do not bridge. L1 (PSU alone, paperclip PS_ON# to an adjacent COM, fan as load) needs BOTH that photo AND a separate explicit 'L1' from the operator. Then L2 (board only, every accessory off the PSU - lighting board M82868-001 and both lighting cables are the prime suspects), then L3 (both 4-pin EPS unplugged, decisive on firmware-vs-electrical). One step, one report, no stacking. Jumper and USB+hotkey classes remain CLOSED. Cap stays on the left pair, cord stays out. Lighting the USB requires POST: promise diagnosis, not a fix. After a real recovery only: Escape=ZBM, F10=BIOS, 3733 XMP, efivarfs ro.",
