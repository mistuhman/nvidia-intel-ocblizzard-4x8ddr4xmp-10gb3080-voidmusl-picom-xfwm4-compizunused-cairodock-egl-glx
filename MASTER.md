{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "DO NOT POWER ON until docs/next-chat-last-power-on.md is read. CLOSED: PB, CMOS, pump, and validated clean-stick recovery checks all failed with the same immediate cycle; do not repeat them. The next free discriminator is one of the two untested physical USB sticks: identify it in Windows without formatting, copy and verify the existing HP recovery tree, safely eject, then power on once with that stick and no hotkeys. Stop on either LED/read activity or the same no-activity cycle. FDO/PSWD/BBR remains optional only if the operator explicitly opts in. After a real flash: Escape=ZBM, F10=BIOS, 3733 XMP, efivarfs ro; no setup_var from OS.",
