{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "OPERATOR GATE: A1b PASS - bench POSTs to code AA on the OMEN PSU M83827-001, so the PSU is EXONERATED (confirm only that the Thermaltake was fully disconnected during the run). Firmware is dead as a cause: a board that trips before it executes cannot be fixed by any jumper, stick, or EFI flasher. LAST FREE STEP: L2 on the OMEN - reinstall the PSU, connect ONLY the 24-pin plus both 4-pin CPU plugs, nothing else (no GPU, no SATA/Molex, no lighting board, no fan hub), one button press. Still instant-cycling receipts the board/CPU verdict; not cycling means an accessory branch is the fault and we re-add one branch per power-on. After L2 the free ladder ENDS: board-vs-CPU cannot be split on the hardware owned (bench is DDR3/older socket, so the 12700KF and the DDR4 DIMMs cannot be cross-tested). Do not turn that limit into a purchase recommendation without the operator asking. Jumper and USB+hotkey classes remain CLOSED. Lighting the USB requires POST.",
