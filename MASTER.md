{
    ...
    "activeObjective": {
        ...
        "lessons": [
            ...
            "compiz 0.8.18 silent rc0 map (source-verified from compiz-reloaded v0.8.18): startup failures print and return rc1; silent rc0 means the event loop ended via shutDown - SIGINT/SIGTERM handler, or handleSelectionClear when another process seizes the WM selection; SIGSEGV instead execs xfwm4 --replace as fallback (launchFallbackWM, printf lost when stdout is redirected) - so empty log + rc0 means killed-or-replaced, never nothing-happened; and compiz-profile-verify SAFE does not prove a live start"
        ],
        "knownRisks": [
    ...
    "liveIncident": {
        ...
        "silentRc0Assessment": "RESOLVED by wave-1 receipts + source review (git clone compiz-reloaded v0.8.18): /usr/bin/compiz is the pristine compiz-core 0.8.18_3 ELF (sha256 ffc93204..., ELF header sane, --help correct, no PATH shadow, xbps state installed) - binary substitution DISPROVEN. Source map: every startup failure prints a message and returns rc1; the ONLY silent rc0 paths are the event loop ending because shutDown was set - via the SIGINT/SIGTERM handler, or via handleSelectionClear when another process seizes the WM selection. SIGSEGV does NOT fit either receipt (its handler prints and execs xfwm4 --replace, which would leave the process alive as xfwm4). Conclusion: compiz is being killed (SIGTERM/SIGINT) or selection-stolen within seconds of every start - login, setsid clean-room, and foreground relight all died identically",
        "wave1Blocks": [
        ...
        "wave1Receipts": "probe 2026-08-22 07:48 UTC: uid=1000(sd), uptime 1:05 (boot 06:43), / 11% used, DISPLAY :0.0 x11; live WM xfwm4 pid 4183 (--sm-client-disable --replace, the operator fallback), NO compiz/emerald/picom/mpv/xwinwrap running; compiz binary identity pristine (above); xrandr healthy: NVIDIA-0 provider, DP-2 primary 2560x1440+0+0 @120, DP-0 1920x1080+2560+0 inverted @119.98, both Brightness 1.0 (GAMMA NOT probed - wave-2 does); displays.xml matches live randr, rewritten at login 06:44; xfce4-session.xml Client0_Command still /home/sd/.local/bin/compiz-session; compiz-profile-verify VERDICT: SAFE (7/7 keys, plugin floor, active hash 27b48d3f, golden af457926, picom slot empty) and compiz-persist-arm: OK already armed - so persistent config is NOT the cause; what changed in 8h: only theme tooling (gunmetal/tiger 06:07-06:10), fleasion-launch 218B rewritten 06:44 AT LOGIN, compiz-guard outputs.wanted 32B 06:48, displays.xml 06:44 - no compiz config drift; Xorg.0.log has zero EE lines. relight receipt: foreground /usr/bin/compiz --debug --sm-disable --replace ccp returned instantly, printed NOTHING, shell echoed COMPIZ_LIVE_RELIGHT_RC_0; operator verbatim verdict: no diff on visuals. Interpretation: config-SAFE does not equal runtime-SAFE; compiz died silently all three times (login, clean-room, relight)",
        "attribution": "OPEN pending wave-2: hypothesis A (primary): a reboot-durable SIGTERM killer - something autostarted or scheduled TERMs any process named compiz within seconds; fits all three deaths incl. clean-room leaving selection owner None (wmctrl BadWindow). Hypothesis B: WM-selection seizer that then exits or races. Origin hypothesis for the original click incident: compiz hit SIGSEGV during the xfce4-display-settings RandR change and its handler launchFallbackWM exec'd xfwm4 --replace (printf lost when stdout redirected), morphing the compiz process into xfwm4 - explains instant compiz-to-xfwm4 switch with no operator action; wave-2 greps .xsession-errors for the fallback line. Black main screen: separate unknown - gamma unprobed (wave-2 probes + one harmless gamma normalize on DP-2) and the xwinwrap+mpv wallpaper layer is dead (both processes absent), root layer unpainted under xfwm4",
        "wave2Blocks": [
            "etc/compiz-wound-wave2.block (user; read-only suspects: autostart desktop files incl. fleasion/lama/Dl/picom/xmb, fleasion-launch 218B rewritten at login, Froststrap, crontab, session cache ~/.cache/sessions, /tmp login trace + repair log, .xsession-errors fallback-grep, gamma+DPMS with one harmless DP-2 gamma reset, GLX sanity, compiz --version; then ONE live strace run of compiz --replace whose trace prints any killer signal with si_pid/si_uid, and aftermath pgrep+wmctrl+ps to reveal any selection thief)"
        ],
        "rebootRule": "no reboot until compiz-profile-verify verdict SAFE AND compiz-persist-arm reports armed AND a live compiz start has been witnessed staying up on target receipts (config-SAFE proved insufficient - login/relight died silently); xfce4-session Failsafe Client0_Command must point at compiz-session for next login",
        "gate": "wave-1 receipts received 2026-08-22; wave-2 block dispatched (paste-proof PASS, block-lint PASS); no fix wave is authored until wave-2 receipts arrive (operator hard rule 2026-08-21: never send a second wave of target commands until the first wave output has arrived)"
    },
    "parked": [
