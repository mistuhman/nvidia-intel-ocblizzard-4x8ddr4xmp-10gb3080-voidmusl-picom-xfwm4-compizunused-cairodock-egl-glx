{
    ...
    "activeObjective": {
        ...
        "nextGateAskFirst": "STOCK BASELINE IN FLIGHT (06:20 UTC). Next gate = operator pastes: (1) Geekbench CPU result URL + Single-Core/Multi-Core lines; (2) Geekbench Compute URL + the compute API actually used (Vulkan vs OpenCL) + GPU score - API must be pinned identical for every later run or the OC delta is meaningless; (3) Superposition 1080p Extreme score (NOT in the operator's stock script - still owed for the stock row); (4) peak PkgWatt/Bzy_MHz/PkgTmp from a --Summary turbostat file and peak pwr/gtemp/pclk from an nvidia-smi dmon file. Meters must be file-backed in /home/sd/oc-meters/ and summarized, never cat-then-rm. After the stock row is complete: BIOS 50P/40E @1.28V, then re-run the identical set and fill the same scoreboard row.",
